...
def check_auth(userinfo):
	...
	if not curr_pass:
		return returned(False,"This user doesn't exist.", "This mail doesn't exist.")
	curr_pass = curr_pass["password"]
	checked = hasher(password, curr_pass[0])
	if curr_pass == checked:
		return returned(True, "authentification was a success.")
	else:
		return returned(False, "Other password", "Invalid password")
...
def reg():
	...
	if email_validator(userinfo["email"]):
		if write_userinfo(userinfo):
			return returned(True, "The registration was a succsses.")
		else:
			return returned(False,"Must contain letters digits and _" "unresolved characters")
	else:
